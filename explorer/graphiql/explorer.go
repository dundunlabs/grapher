...
var _ grapher.Explorer = (*Explorer)(nil)

func NewExplorer() *Explorer {
	tmpl := template.Must(template.New("graphiql").Parse(graphiqlHTML))
	return &Explorer{tmpl: tmpl}
}

func (e *Explorer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	data := map[string]any{"url": r.URL.Path}
